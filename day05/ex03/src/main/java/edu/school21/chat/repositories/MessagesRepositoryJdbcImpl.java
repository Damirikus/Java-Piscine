...
public class MessagesRepositoryJdbcImpl implements MessagesRepositories {
    ...
    public Optional<Message> findById(Long id) {
        ...
        try {
            ...
            if (resultSet.getTimestamp("send_date") == null){
    ...
    public void update(Message message) {
        ...
        try {
            String sql = "UPDATE messages SET author=?, room=?, message=?, send_date=? WHERE message_id=?";
            PreparedStatement statement = DataSource.getConnection().prepareStatement(sql);

            statement.setLong(1, message.getAuthor().getUserId());
            statement.setLong(2, message.getRoom().getChatroomId());
            statement.setString(3, message.getText() == null? null : message.getText());
            statement.setTimestamp(4, message.getDate() == null? null : Timestamp.valueOf(message.getDate()));
